...
- ✅ теги — атомарные тесты: создание и наличие в списке / переименование (id неизменен) / удаление
...
./gradlew test mobile_test              # все 39 тестов в один Allure-отчёт
```
...
- **вручную** (Run workflow) запускается любой слой: `api_test` / `web_test` / `mobile_test` /
  `test` (api+web) / `all` (все 39 тестов одним прогоном в общий отчёт);
...
│   └── ApiErrors   — константы кодов и текстов ошибок API
├── web/pages       — Page Object'ы веба (HomePage, LoginPage, RegisterPage, TasksPage)
├── config          — Owner-конфиги (api / web / browserstack)
├── helpers         — TestUsers (общий пользователь), TestData (генерация данных задач/тегов),
│                     BrowserSession (localStorage-авторизация), Attachments (видео BrowserStack)
└── tests
    ├── api         — 17 тестов (Auth, User, Tasks, Tags)
...
└── tests/mobile    — 5 смоук-тестов (Onboarding, Auth, Tasks)
```
