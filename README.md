...
./gradlew test mobile_test              # все 38 тестов в один Allure-отчёт
```
...
- **вручную** (Run workflow) запускается любой слой: `api_test` / `web_test` / `mobile_test` /
  `test` (api+web) / `all` (все 38 тестов одним прогоном в общий отчёт);
...
└── tests
    ├── api         — 16 тестов (Auth, User, Tasks, Tags)
